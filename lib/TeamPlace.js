...
class TeamPlace {
  ...
  createList(response) {
    ...
    if (!query) {
      return response.reply('Enter the name of your new list e.g. @lunchy create list Donuts');
    }

    response.reply(`Creating new list "${query}"`);
    this.store.createList(query);
  }
  ...
  _handleFoundRestaurantsDialog(response, restaurants) {
    ...
    response.reply(
      ...
      restaurants.map(({ name, url, location, user_rating }, index) =>
      `${index+1}) *${name}* - _${location.address}_ * Rating: ${user_rating.aggregate_rating}* `
        + ':star:'.repeat(Math.round(user_rating.aggregate_rating))).join('\n')
    );
