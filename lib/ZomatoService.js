...
class ZomatoService {
   ...
   * @private
     */
  _search(params, endpoint = 'search') {
    return this._getCached(`/${endpoint}${this._query(params)}`);
  }
  ...
  searchCities(query, maxCount = 10) {
    ...
    return this._search(params, 'cities')
  ...
  searchRestaurants(query, maxCount = 10, locationId = this.DEFAULT_LOCATION_ID) {
    ...
    return Promise.all(promises).then((results) => {
      const restaurants = results.map((result) =>
        result.restaurants && result.restaurants.map(({restaurant}) => restaurant));
